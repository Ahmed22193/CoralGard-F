...
function updateData(newData) {
  ...
  currentData = { healthy: h, bleached: b, dead: d };

  // إجمالي مضبوط (لو عايز تقرّبه)
  const total = Math.round((h + b + d) * 100) / 100;
  totalPercent.textContent = `${total}%`;

  // ترند آمن: قيمة رقمية + تمثيل نصي
  const trendVal = h - 70;
  const trendText = `${trendVal > 0 ? "+" : ""}${trendVal}%`;
  trendIndicator.textContent = trendText;

  // ما تمسحش كل الكلاسات
  trendIndicator.classList.remove("text-green-600", "text-red-600");
  trendIndicator.classList.add(
    trendVal >= 0 ? "text-green-600" : "text-red-600",
    "text-sm",
    "font-medium"
  );
  ...
  setTimeout(() => {
    pieChart.classList.add("pie-animate");
    linePath.classList.add("line-animate");
    fillPath.classList.add("fill-animate");
    console.log("updatePieChart data : ", currentData);
...
function updatePieChart(data) {
  const total = data.healthy + data.bleached + data.dead;
  const radius = 15.9155;
  const circumference = 2 * Math.PI * radius;

  const healthyValue = (data.healthy / total) * circumference;
  const bleachedValue = (data.bleached / total) * circumference;
  const deadValue = (data.dead / total) * circumference;

  const deadPath = document.querySelector(".dead-path");
  const bleachedPath = document.querySelector(".bleached-path");
  const healthyPath = document.querySelector(".healthy-path");

  // Dead
  deadPath.setAttribute(
    "stroke-dasharray",
    `${deadValue} ${circumference - deadValue}`
  );
  deadPath.setAttribute("stroke-dashoffset", 0);

  // Bleached
  bleachedPath.setAttribute(
    "stroke-dasharray",
    `${bleachedValue} ${circumference - bleachedValue}`
  );
  bleachedPath.setAttribute("stroke-dashoffset", -deadValue);

  // Healthy
  healthyPath.setAttribute(
    "stroke-dasharray",
    `${healthyValue} ${circumference - healthyValue}`
  );
  healthyPath.setAttribute("stroke-dashoffset", -(deadValue + bleachedValue));

  // Total %
  document.getElementById("total-percent").innerText = `${Math.round(
    (total / total) * 100
  )}%`;
}

function resetCharts() {
  pieChart.classList.remove("pie-animate");
  linePath.classList.remove("line-animate");
  fillPath.classList.remove("fill-animate");

  const paths = document.querySelectorAll(
    ".healthy-path, .bleached-path, .dead-path"
  );
  paths.forEach((path) => {
